...
def stack_mnist(data_dir, num_training_sample, num_test_sample, imageSize):
    ...
    trX = loaded[16:].reshape((10000, 28, 28, 1)).astype(np.float) #changes
    fd.close()

    fd = open(os.path.join(data_dir, 'raw/t10k-images-idx3-ubyte'))
    loaded = np.fromfile(file=fd, dtype=np.uint8)
    teX = loaded[16:].reshape((10000, 28, 28, 1)).astype(np.float) #changes
    fd.close()

    # Load MNIST labels
    fd = open(os.path.join(data_dir, 'raw/train-labels-idx1-ubyte'))
    loaded = np.fromfile(file=fd, dtype=np.uint8)
    trY = loaded[8:].reshape((10000)).astype(np.float) #changes 
    fd.close()
